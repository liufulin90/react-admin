...
class RequestPage extends React.Component {
  constructor (props) {
    super(props)
    const {currntRequest} = this.props
    this.state = {
      currntRequest: currntRequest,
      method: 'get',
      result: '',
    }
  }
  ...
  handleRequest = () => {
    ...
    const { desc, url, ...requestParams } = currntRequest
    this.setState({
      ...this.state,
    ...
    //   }
    // }))
    request(url, { ...requestParams }).then((data) => {
...
function mapStateToProps ({}) {
  return {currntRequest: requestOptions[0]}
}
